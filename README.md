...
点击 macOS 菜单栏或 Windows 系统托盘中的 Codex Quota 项，可显示或隐藏悬浮层、开启或关闭开机启动、按 2 pt/px 调整位置、恢复默认位置、选择跟随系统/深色/浅色外观、查看关于信息或退出应用。macOS 菜单中还可打开辅助功能设置。开机启动默认关闭。
...
- Windows 胶囊位置不准：确认前台进程是官方 `ChatGPT`/`Codex`，再从托盘菜单微调或恢复默认位置。
